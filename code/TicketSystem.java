...
public class TicketSystem {
	...
	void inputData(int position, int companionPrint, ArrayList<OrderData> orderList) {
		...
		orderItem.ticketClass = input.orderTicketClass(); // 이용권
		orderItem.ticketType = input.orderTicketType(); // 권종
		orderItem.citizenNum = input.orderCitizenNum(); // 주민번호
		manAge = cal.calcAge(orderItem.citizenNum, nowTime); // 주민번호 -> 만나이
		orderItem.ageGroup = cal.calcAgeGroup(manAge); // 만나이 -> 나이그룹
		...
		if (orderItem.preferClass != 1) { // 만약 우대사항이 있다면 할인금액으로 변경
			...
			if (companionIndex == 2 || companionIndex == 3 || companionIndex == 4) {
				...
				orderItem.price = cal.calcDiscount(orderItem.price, companionIndex, orderItem.ticketClass); // 동반할인금액으로 변경
